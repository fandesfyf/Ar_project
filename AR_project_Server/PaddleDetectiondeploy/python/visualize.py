...
def visualize_box_mask(im, results, labels, mask_resolution=14):
    ...
    if 'boxes' in results:
        im = draw_box(im, results['boxes'], labels, results["draw_carplate"])
    return im
...
def draw_box(im, np_boxes, labels, draw_carplate: list):
    """ 
    Args:
        im (PIL.Image.Image): PIL image
        np_boxes (np.ndarray): shape:[N,6], N: number of box，
    ...
    for dt in np_boxes:
        ...
        # draw label
        if labels[clsid] == "carplate" and len(draw_carplate) > 0:
            dctext = draw_carplate.pop(0)
            # print(dctext)
            if dctext=="notfound":
                dctext=""
            text = "{}:{} {:.2f}".format(labels[clsid], dctext, score).encode("utf-8")
        else:
            text = "{}: {:.2f}".format(labels[clsid], score).encode("utf-8")
        tw, th = draw.textsize(text)
        draw.rectangle(
            [(xmin + 1, ymin - th), (xmin + tw + 1, ymin)], fill=color)
        draw.text((xmin + 1, ymin - th), str(text,encoding="utf-8"), fill=(255, 255, 255),
                  font=ImageFont.truetype(font="simsun.ttc", size=15, encoding="utf-8"))
    return im
